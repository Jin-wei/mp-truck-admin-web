            ...
            <div class="col-md-12">
                <div class="panel panel-inverse">
                    <div class="panel-heading">
                        ...
                        <h4 class="panel-title">角色/组维护</h4>
                    </div>
                    <div class="panel-toolbar">
                        <div class="form-inline">
                            <div class="form-group">
                                <div class="form-group">
                                    <button id="addFolder" class="btn btn-primary btn-info" v-on:click="addDep"><i
                                            class="glyphicon glyphicon-plus"></i> 增加权限组
                                    </button>
                                    <button id="addGroup" class="btn btn-primary btn-info" v-on:click="addPos"><i
                                            class="glyphicon glyphicon-plus"></i> 增加角色
                                    </button>
                                    <button id="editNode" class="btn btn-primary btn-info" v-on:click="editNode"><i
                                            class="glyphicon glyphicon-plus"></i> 编辑
                                    </button>
                                </div>
                ...
                <div class="modal-content">
                    ...
                    <form @submit.prevent="submitDep" id="formDep">
                        <div class="modal-body">
                            <div class="form-group">
                                <label>组名称</label>
                                <input class="form-control" v-model="nameA" data-parsley-required="true" maxlength="50"
                ...
                <div class="modal-content">
                    ...
                    <form @submit.prevent="submitPos" id="formPos">
                        <div class="modal-body">
                            ...
                            <div class="form-group">
                                <label>角色级别</label>
                                <select class="form-control select2" id="role_group_level"
...
<script>
    ...
    const apiUrl = '/api/common/system/RoleGroupControl?method=';
        ...
        mounted: function () {
            ...
            async function initPage() {
                try {
                    ...
                    common.initSelect2($('#role_group_level'), retData.roleLevel);
            ...
            submitDep: async function (event) {
                ...
                try {
                    if ($('#formDep').parsley().isValid()) {
                        if (_self.act === '1') {
                            let workRow = {
                                'role_group_name': _self.nameA,
                                'role_group_level': common.getSelect2Val('role_group_level'),
                                'parent_id': _self.actNode.role_group_id,
                                'node_type': '00'
                            };
                            await _self.$http.post(apiUrl + 'a', workRow)
                        } else if (_self.act === '2') {
                            let workRow = {
                                'role_group_id': _self.actNode.role_group_id,
                                'role_group_level': common.getSelect2Val('role_group_level'),
                                'role_group_name': _self.nameA
                            };
                            await _self.$http.post(apiUrl + 'm', workRow);
                            $('#DepModal').modal('hide');
                ...
                try {
                    ...
                    if (nodeObj[0].node_type === '00') {
                        ...
                        async function getCheckData() {
                            let response = await _self.$http.post(apiUrl + 'gc', {
                                role_group_id: nodeObj[0].role_group_id
                            });
                            ...
                            for (let i = 0; i < nodes.length; i++) {
                                for (let j = 0; j < retData.groupMenu.length; j++) {
                                    if (nodes[i].menu_id === retData.groupMenu[j]) {
