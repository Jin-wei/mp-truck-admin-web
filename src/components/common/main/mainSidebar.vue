                        ...
                        <template v-if="item.node_type === '00'">
                            <li class="has-sub" @click="menuClick(index)" :class="[{active: (index == curIndex)}]">
                                <a href="javascript:;">
                                    ...
                                    <span class="label label-theme count-badge pull-right" v-if="item.menu_name === `基础信息管理` && isShowBadge && badgeNum > 0">{{badgeNum}}</span>
                                        ...
                                        <template v-if="sItem.node_type === '01' ">
                                            <li @click="subMenuClick(subIndex)" :class="[{active: (subIndex == subCurIndex && lastIndex == curIndex)}]">
                                                <router-link :to="sItem.menu_path"> {{ sItem.menu_name }}
                                                    <span class="label label-theme count-badge pull-right" v-if="sItem.menu_name === `车辆管理` && badgeNum > 0">{{badgeNum}}</span>
...
<script>
    ...
    export default {
        ...
        mounted: function () {
            ...
            bus.$on('subMenuChanged',function (msg) {
                _self.subCurIndex += 1;
            });
            getBadgeNum();
        },
